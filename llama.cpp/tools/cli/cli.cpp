...
#include "thermokv_mock.h"
ThermoKVMockHAL mock_phone;
            ...
            if (res_partial) {
                ...
                for (const auto & diff : res_partial->oaicompat_msg_diffs) {
                    if (!diff.content_delta.empty()) {
                        ...
                        // --- ThermoKV Mock Router Injection ---
                        static float fake_temp = 35.0f;
                        static int drop_start = 50; 
                        fake_temp += 0.5f; 
                        
                        if (fake_temp >= 80.0f) {
                            int drop_end = drop_start + 100;
                            printf("\n\n[ThermoKV: %.1fC | STATE C: Dropping tokens %d to %d to shed RAM load!]\n\n", fake_temp, drop_start, drop_end);
                            
                            llama_memory_t current_memory = llama_get_memory(ctx_server.get_llama_context());
                            
                            // 1. Delete the tokens for sequence 0
                            llama_memory_seq_rm(current_memory, 0, drop_start, drop_end);
                            
                            // 2. Shift all tokens after the hole to the left for sequence 0
                            llama_memory_seq_add(current_memory, 0, drop_end, -1, -(drop_end - drop_start));
