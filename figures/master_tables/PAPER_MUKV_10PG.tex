...
\begin{table}[tb]
\caption{Adreno 840, 12K-token prompt plus 4096 generated tokens, f16 KV for every policy, cold start. Median of $n$ runs. Cells: retained after prefill. Phi-3 rows come from the current build, whose driver gate is described in the text.}
\label{tab:gpu}
\centering\footnotesize
...
\begin{table}[tb]
\caption{Needle retrieval on the phone, 392 cells: hits out of 14 depths, 7 at 4K and 7 at 8K, per model. Cells attended: mean live cache after prefill, over all 56 cells of a policy. A quality axis only.}
\label{tab:niah}
\centering\footnotesize
\setlength{\tabcolsep}{4pt}
