...
\caption{The energy-aware control plane, once per request. Circled numbers are the components of Figure~\ref{fig:architecture}, so the same part carries the same number in both figures. Run is that figure's steps 1 to 3. A number appears on two boxes where one stage spans two components.}
