...
\node[head,anchor=north west,text=gr] at (1.30,3.7) {THERMAL WATCHDOG \quad {\mdseries 2\,Hz, one step at a time, never raises}};
...
\draw[data] (sens) -- (appl);
\draw[vend] (3.90,0.28) -- (4.40,0.28); \node[lab,anchor=west,text=gr,font=\tiny] at (4.45,0.28) {vendor limiter, applied last};
\node[lab,anchor=west,text=gr,font=\tiny] at (10.30,0.28) {DDR $\ge$ 71\,\textdegree C also floors both CPU clusters to 1267};
% ---- CPU ladder: battery C -> prime-core cap
\node[head,text=or,anchor=west] at (3.90,3.42) {CPU cap, both clusters \; {\mdseries lower of the two rows}};
...
\foreach \x/\t in {4.57/50.0, 5.91/51.0, 6.58/51.5, 7.25/52.0, 7.93/52.5, 8.60/52.8}
  {\node[axl,anchor=north] at (\x,0.55) {\t};}
\node[axl,anchor=west] at (9.05,0.83) {battery};
\node[axl,anchor=west] at (9.05,0.55) {skin};
% ---- GPU ladder: DDR C -> Adreno cap
\node[head,text=or,anchor=west] at (10.30,3.42) {GPU cap \; {\mdseries by DDR \textdegree C, 1.5\,\textdegree C hysteresis}};
...
\draw[heat] (0.90,4.35) -- (sens.north -| 0.90,0) node[lab,text=gr,pos=0.55,right=1pt] {heat};
