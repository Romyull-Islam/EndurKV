...
\node[box]  (run)     at (12.05,3.7) {\bx{Run}{prefill, score, select,\\ compact, decode\\ (steps 1 to 3, Figure~1)}};
\node[mukv] (measure) at (15.5,3.7){\bx{Measure}{USB rail +\\ coulomb counter\\ per phase}};
\node[stepg] at (sense.north west) {A};\node[stepn] at (decide.north west) {B};\node[stepn] at (act.north west) {C};
\node[stepg] at (run.north west) {D};\node[stepn] at (measure.north west) {E};
...
\node[stepn] at (learn.north west) {F};
