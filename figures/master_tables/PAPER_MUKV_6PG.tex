...
\caption{\muKV\ on the phone. Top: the inference pass; scoring, selection and compaction (1 to 3) are the only changes. Bottom: the watchdog, which steps each clock cap down a ladder anchored above the vendor cliff.}
...
\caption{Live KV cells through prefill and decode, Llama-3.2-1B on the phone CPU. Every policy builds the full prompt cache. Neither per-head evictor returns to its budget; \muKV\ compacts to 721 and holds near 2K.}
...
\begin{table}[tb]
\caption{Adreno 840, Llama-3.2-1B, ctx 16384, 9737-token prompt, 4096 generated, f16 KV, median where $n>1$. Rows group by how a policy selects. The $n{=}3$ and $n{=}1$ rows are separate campaigns whose vanilla cells agree within 0.5\%.}
\label{tab:gpu}
