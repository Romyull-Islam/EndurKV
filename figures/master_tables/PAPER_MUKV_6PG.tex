...
\item \textbf{Budgets do not materialize.} One cell array serves every head and layer, so a cell is freed only when every head and every layer drops it. SnapKV reports an 8.2 times smaller cache at 16K inputs~\cite{li2024snapkv}; at its own budget on the phone it keeps 94\% of the cells. H2O reports 5 to 10 times at a 20\% budget~\cite{zhang2023h2o}; at that budget it keeps 92\%. Across the four per-head policies, 36 to 68\% of the cache stays live at a nominal 1024.
