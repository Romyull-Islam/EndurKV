...
\paragraph{Heat.} The full cache on Bonsai-8B runs 85.6 minutes and drives the battery to 50.2\C, where the vendor deep-throttles both clusters to 883\,MHz. \muKV\ finishes in 33.2 minutes at a 44.2\C\ peak, before the trigger, so it never meets the cliff the baseline spends 923\,s inside. The watchdog stayed dormant in every cell of Table~\ref{tab:cpu}, which makes that result eviction alone. We also tied the cache budget itself to DDR temperature, over a four-tier ladder, and the result is negative. Halving $K$ under co-load left peak DDR unchanged, and on a cool phone a smaller cache runs hotter at peak, because the saved bandwidth converts to speed and power. Cache size controls throughput, time and energy. The clock caps heat.
