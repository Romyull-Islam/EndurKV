...
HOST=${HOST:-/tmp/snapkv_gpu_canonical}; mkdir -p $HOST
...
cell(){ local TAG=$1 ARM=$2; local D=$HOST/$TAG; [ -f "$D/meta.json" ] && { echo "  [$TAG] cached"; return; }; mkdir -p $D
  ...
  adb_safe_shell "su -c 'rm -rf $ROOT/snapkvtest/$TAG; mkdir -p $ROOT/snapkvtest/$TAG; echo 0 > /sys/class/kgsl/kgsl-3d0/max_pwrlevel; echo 1200000000 > /sys/class/kgsl/kgsl-3d0/max_gpuclk; rm -f $ROOT/gpu_watchdog.log'" < /dev/null >/dev/null 2>&1
  ...
  adb_safe_shell "su -c 'nohup sh /data/local/tmp/sample_sensors.sh --out $ROOT/snapkvtest/$TAG/sensors.csv --hz 2 >/dev/null 2>&1 &'" < /dev/null >/dev/null 2>&1
  adb_safe_shell "su -c 'cd $BIN && LD_LIBRARY_PATH=$BIN nohup taskset f0 nice -n -20 ./eviction_bench --model $MODEL --prompt $P --prompt-id $TAG --eval-mode gen --max-tokens 4096 --ignore-eos --ctx-size 16384 --seed 42 --threads 4 --n-gpu-layers 99 --greedy --cache-type-k f16 --cache-type-v f16 --policy snapkv --obs-window 64 --n-sink 0 --k-nominal 1024 --n-batch 512 --n-ubatch 64 --out-meta $ROOT/snapkvtest/$TAG/meta.json --out-gen $ROOT/snapkvtest/$TAG/gen.txt --out-csv /dev/null > /dev/null 2> $ROOT/snapkvtest/$TAG/err &'" < /dev/null >/dev/null 2>&1
  for i in $(seq 1 120); do sleep 10; adb_safe_shell "[ -s $ROOT/snapkvtest/$TAG/meta.json ] && echo done || echo run" < /dev/null | grep -q done && break; done
  sleep 3; adb_safe_shell "su -c 'pkill -f sample_sensor[s]'" < /dev/null >/dev/null 2>&1; wd_stop
  for f in meta.json err sensors.csv; do adb_safe_pull $ROOT/snapkvtest/$TAG/$f $D/$f >/dev/null 2>&1; done
...
for R in 1 2 3; do cell snapkv_canon_r$R off; done
echo SNAPKVGPU_DONE
