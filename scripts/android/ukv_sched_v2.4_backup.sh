...
PROMPT=""; MAXTOK=""; SIZE=""; FSOC=""; FSTAT=""; CPUONLY=0; IGNEOS=""; DRY=0; TAG=""; LEVER=""; FEEDBACK=1; MODELARG=""
while [ $# -gt 0 ]; do
  case "$1" in
...
# --model NAME (a file under $ROOT/models) or a path; default Llama-3.2-1B. v2.4 (2026-09-08).
if [ -n "$MODELARG" ]; then case "$MODELARG" in */*) MODEL=$MODELARG;; *) MODEL=$ROOT/models/$MODELARG;; esac; fi
[ -f "$MODEL" ] || { echo "model not found: $MODEL" >&2; exit 1; }
MNAME=$(basename $MODEL)
# Per-model backend capability, learned from failures: "<model file> gpu no <reason>". A model listed
...
grep -q "^$MNAME gpu no" $CAP 2>/dev/null && { GPU_OK=0; echo "[sched] $MNAME is marked gpu=no in $CAP: CPU plans only" >&2; }
...
BANGS=0; [ -s $OUT/gen.txt ] && BANGS=$(awk 'BEGIN{b=0;n=0} { n+=length($0); b+=gsub(/!/,"") } END{ if (n>0) printf "%d", 100*b/n; else print 0 }' $OUT/gen.txt)
INVALID=""
[ $RC -ne 0 ] && INVALID="bench rc=$RC"
[ -s $OUT/meta.json ] || INVALID="${INVALID:-no meta.json}"
[ "$BANGS" -ge 20 ] && INVALID="${INVALID:-degenerate output, ${BANGS}% bangs}"
if [ -n "$INVALID" ]; then
  echo "[sched] run invalid on $BACKEND: $INVALID" >&2
  if [ "$BACKEND" = gpu ]; then
    grep -q "^$MNAME gpu no" $CAP 2>/dev/null || echo "$MNAME gpu no $INVALID ($(date '+%F %T'))" >> $CAP
    echo "[sched] $MNAME marked gpu=no; re-running this request on the CPU" >&2
    exec sh $0 --prompt $PROMPT --cpu-only --tag ${TAG}_cpu ${MODELARG:+--model $MODELARG} ${MAXTOK:+--max-tokens $MAXTOK} ${SIZE:+--size $SIZE} ${LEVER:+--lever $LEVER} ${FSOC:+--force-soc $FSOC} ${FSTAT:+--force-status $FSTAT} $IGNEOS $( [ "$FEEDBACK" = 0 ] && echo --no-feedback )
  fi
  exit 1
fi
