...
OUT_HOST=/tmp/nat_cpu; mkdir -p "$OUT_HOST"
TS=$(date +%Y%m%d_%H%M%S); OUT=/data/local/tmp/endurkv/logs/natcpu_$TS
...
touch /tmp/nat_cpu_DONE; echo "[$(date +%H:%M:%S)] NATURAL CPU DONE -> $OUT_HOST"
