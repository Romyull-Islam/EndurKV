...
OUT_HOST=/tmp/wd_control; mkdir -p "$OUT_HOST"
OUT=/data/local/tmp/endurkv/logs/wdctrl_$(date +%s 2>/dev/null || echo run)
...
adb_safe_shell "su -c 'rm -f $LOG_STOP $WD_STOP'" < /dev/null   # clear stale stop-flags (control has no watchdog line to do this)
adb_safe_shell "su -c 'nohup sh /data/local/tmp/wd_logger.sh $OUT/wd_trace.csv $LOG_STOP $BZ $SZ >/dev/null 2>&1 &'" < /dev/null
sleep 2

echo "[$(date +%H:%M:%S)] running Bonsai muKV WITHOUT watchdog (control)"
...
adb_safe_shell "su -c 'touch $WD_STOP $LOG_STOP; sleep 2; for c in cpu6 cpu7; do echo 2438400 > /sys/devices/system/cpu/\$c/cpufreq/scaling_max_freq; done; echo 1 > /sys/class/oplus_chg/battery/mmi_charging_enable'" < /dev/null
...
touch /tmp/wd_control_DONE
echo "[$(date +%H:%M:%S)] WATCHDOG CONTROL (no-wd) DONE -> $OUT_HOST"
