...
#
# ---- 2026-08-19 DERIVATIVE: FA-ON baselines, as their own papers describe -----
# Regenerated from run_definitive_cpu_keydiff.sh so it inherits every robustness fix
# made on 08-18/19 (detached device-side execution with a hard timeout, authoritative
# cool gate, cell-level resume, bounded adb calls and pushes). The earlier hand-built
# version predated all of them and would have lost a cell on every tunnel drop.
#
# WHAT THIS MEASURES. Our harness gave every attention-scored baseline ONE generic
# capture mechanism: an eval-callback with FlashAttention disabled for the whole
# prefill. No baseline's paper asks for that. Per KeyDiff App. A.1:
#   SnapKV  phi(A) = (A^T 1) * K over the LAST window queries only -- its own code
#           computes matmul(query[..., -w:, :], key^T) ALONGSIDE normal attention.
#   Ada-KV  SnapKV's scores plus a global budget allocator. Same windowed matmul.
# Both select ONCE at end of prefill, so both can be scored from the in-graph side
# node with THEIR OWN window and pooling -- flags only, no new code. TOVA and H2O
# score during DECODE, where the side node cannot fire, so they stay FA-off; that is
# a property of their designs and is reported as such rather than fixed.
#
# ---- inherited from the KeyDiff derivative ----------------------------------
...
# ---- MUST NOT SHARE THE PHONE (2026-08-20) ----------------------------------
# This script was regenerated from the KeyDiff campaign AFTER that campaign's own
# wait-gate had been deleted, so it inherited no gate at all. Two campaigns on one
# phone is not merely slow: on 08-19 two concurrent 6-thread runs of the same cell
# drove the device to 57 C, made the cool gate permanently unsatisfiable, and produced
# two mutually invalid cells. Wait for the KeyDiff table to finish.
...
# Each baseline at ITS OWN published window/pooling, scored from the in-graph side node
# so FlashAttention stays on. FA-off controls run in the SAME session so the delta is
# attributable to the path and not to drift between sessions.
# ---- muKV on THIS build, with the watchdog A/B that has never been run -------
# Two gaps this closes. (1) PROVENANCE: every published muKV CPU number came from
# bin_cpu_sol2, while the KeyDiff cells run on bin_cpu_kd. vanilla_ctl reproduces
# 5.0 tok/s across both, so the builds agree -- but muKV should be measured on the
# same binary as the policy it is being compared against, not inferred across builds.
# (2) SYMMETRY: every muKV CPU cell to date ran with the watchdog ENABLED while every
# baseline ran with it disabled. The watchdog is muKV-only by design, but that makes
# "how much of muKV's advantage is the governor?" unanswerable from the existing data.
# Its log in the natural-equilibrium campaign ends "last tier=0 driver=none threat=0/1000"
# -- it never engaged -- so we expect wd0 == wd1. That is a prediction, and running both
# turns it into a measurement. wd0 is the cell that is apples-to-apples with the baselines.
