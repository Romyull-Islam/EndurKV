...
OUT_DIR=${OUT_DIR:-$WORKDIR/logs/wave6_v1fa_bounded_$(date +%s)}
mkdir -p "$OUT_DIR"
PROG="$OUT_DIR/progress.log"
echo "[$(date)] wave6 v1_FA-bounded START -> $OUT_DIR" > "$PROG"
...
run_v1fa() {
    local cell_dir="$OUT_DIR/v1_fa_K512_bounded"
    mkdir -p "$cell_dir"
    echo "" >> "$PROG"
    echo "[$(date)] === START v1_fa_K512_bounded (Wave-4 protocol + new binary) ===" >> "$PROG"
    cool_phone "v1_fa"
    ...
    while true; do
        ...
        echo "[$(date)]   v1_fa iter=$ITER t=${ELAPSED}s exit=$EXIT decode_tps=$DT peak_kv=$KV evicted=$EV" >> "$PROG"
    done
    kill $SAMPLER 2>/dev/null
    pgrep -f sample_sensors | xargs -r kill 2>/dev/null
    sleep 2
    echo "[$(date)] === DONE v1_fa_K512_bounded: iters=$ITER ===" >> "$PROG"
}
