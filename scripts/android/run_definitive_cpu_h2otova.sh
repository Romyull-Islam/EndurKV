...
#
# ---- 2026-08-23 DERIVATIVE: H2O and TOVA on the CURRENT build ----------------
# WHY. H2O and TOVA are baselines in essentially every KV-cache paper. They existed
# in our tables only on the PRIOR build (bin_cpu_sol2), so the master table either
# mixed builds or omitted them; omitting them is worse -- a reader takes the absence
# of the two most-cited baselines as evasion. vanilla reproduces 5.02 tok/s on both
# builds, so the prior numbers were not wrong, but same-build is strictly better and
# costs two cells.
#
# CONFIGURATION, as their papers describe: both score at EVERY decode step (H2O
# accumulates attention, phi(A)=A_prev + A^T 1; TOVA reads the last query's row,
# phi(A)=A_{-1,:}), which the in-graph side node cannot serve -- it fires only when
# q->ne[1] > 1, i.e. during prefill. So both run FA-off. That is a property of their
# designs, not a handicap we impose, and it is reported as such.
#
# ---- inherited from the FA-on baselines derivative ---------------------------
