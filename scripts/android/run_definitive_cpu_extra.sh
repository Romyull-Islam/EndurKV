...
platform_cap(){ adb_safe_shell "su -c 'pkill -9 -f eviction_bench 2>/dev/null; pkill -9 -f sample_sensors 2>/dev/null; pkill -9 -f preempt_throttle_watchdog 2>/dev/null; touch $WD_STOP; rm -f $CAPHOLD_STOP; nohup sh -c \"while [ ! -f $CAPHOLD_STOP ]; do for c in cpu6 cpu7; do echo 1632000 > /sys/devices/system/cpu/\\\$c/cpufreq/scaling_max_freq 2>/dev/null; done; sleep 5; done\" >/dev/null 2>&1 &'" < /dev/null
  ...
  local T0=$(date +%s)
  while true; do
    m=$(adb_safe_shell "cat /sys/devices/system/cpu/cpu6/cpufreq/scaling_max_freq" < /dev/null | tr -d '\r'); m=${m:-0}
    [ "$m" -le 1632000 ] 2>/dev/null && { echo "  [cap verified ${m}]"; return; }
    [ $(($(date +%s)-T0)) -gt 30 ] && { echo "  [cap NOT verified: ${m}]"; return; }
    sleep 3
  done; }
...
run tova_canon   0 --policy tova_canonical                      # strict TOVA (last-query top-K, no safety net)
run adakv        0 --policy adakv --n-sink 0 --obs-window 32    # RE-RUN: canonical Ada-SnapKV flags (measured row used window16/sink4)
# restore (stop cap-holder first, then native clocks + charging)
adb_safe_shell "su -c 'touch $CAPHOLD_STOP; touch $WD_STOP; sleep 6; for c in cpu6 cpu7; do cat /sys/devices/system/cpu/\$c/cpufreq/cpuinfo_max_freq > /sys/devices/system/cpu/\$c/cpufreq/scaling_max_freq; done; echo 1 > /sys/class/oplus_chg/battery/mmi_charging_enable'" < /dev/null
