#!/system/bin/sh
# μKV Bonsai-8B full-GPU validation, bounded to avoid an unobservable hang.
# Workload matches the CPU WikiText-2 experiment: 10,074-token prompt, ctx 16k.
set -u
...
D="$OUT/mukv_mass_sol2_faon_gpu99_16k"
...
cool > "$D/cold_gate.txt"
sh "$WORK/scripts/sample_sensors.sh" --out "$D/sensors.csv" --hz 5 >/dev/null 2>&1 & SPID=$!
LD_LIBRARY_PATH="$LIB" "$BIN" --prompt "$D/prompt.txt" --prompt-id mukv_mass_sol2_faon_gpu99_16k \
